...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
    ...
    --         on the mutex.
       ...
       -----------  Mutex . HandlerOff  -----------
       -- additional function support to implement hoare condition
       method HandlerOff(ptrToThread: ptr to Thread)
          heldBy = ptrToThread
       endMethod
...
-----------------------------  Condition  ---------------------------------
  ...
  behavior HoareCondition
    ...
    -- means that in the time between a Signal() operation and the awakening
    -- and execution of the corrsponding waiting specified thread, so other threads 
    -- can not have snuck in and run.  
    -- This class provides the following methods:
    --    Wait(mutex)
    --         This method assumes the mutex has alreasy been locked.
    --         It unlocks it, and goes to sleep waiting for a signal on
    ...
    --    Signal(mutex)
    --         If there are any threads waiting on this condition, this
    --         method will wake up the specified thread on the waiting list.
    --         So that thread can wake up and re-quire the mutex and resume execution
    --    Init()
    --         Each condition must be initialized.
      ...
      ----------  HoareCondition . Wait  ----------
      -- implement of hoare condition wait is similarly to condition above
      -- but when we add thread to waiting thread then go to sleep, we dont
      -- lock the mutex
      method Wait (mutex: ptr to Mutex)
          var
            oldIntStat: int
      ...
      -- implement of hoare condition signal is similarly to condition above
      -- but when we have exist thread, you need to signal specified thread by
      -- calling wrapper function HandlerOff, and after we add to ready list,
      -- we can lock the mutex so the current thread can go to sleep 
      method Signal (mutex: ptr to Mutex)
          var
            oldIntStat: int
            t: ptr to Thread
          ...
          if t
            ...
            t.status = READY
            readyList.AddToEnd (t)
            mutex.Lock()  -- lock the mutex so the current thread goes to sleep
          endIf
          oldIntStat = SetInterruptsTo (oldIntStat)
        endMethod
...
-----------------------------  ThreadManager  ---------------------------------
  ...
  behavior ThreadManager
      ...
      method Init ()
        ...
        --Add Thread to free list and set status to unused 
      ...
      method GetANewThread () returns ptr to Thread
        var
          pointerToThread: ptr to Thread
        -- 
        -- This method returns a new Thread; it will wait
        -- until one is available.
        -- 

        threadManagerLock.Lock()  -- lock the mutex              
        
        if (freeList.IsEmpty()) -- free list is empty, you need to wait             
          -- wait on condition of thread become free
          threadBecameFree.Wait(&threadManagerLock)
        endIf

        -- has some thread in free list, can remove it from free list
        pointerToThread = freeList.Remove()

        -- create a new thread
        pointerToThread.status = JUST_CREATED      
        threadManagerLock.Unlock() -- unlock the mutex
      ...
      method FreeThread (th: ptr to Thread)
        ...
        threadManagerLock.Lock() -- Lock the mutex
        currentThread.status = UNUSED  -- change status back to unused
        freeList.AddToEnd(th)  -- add this pointer to list of free thread
        threadBecameFree.Signal(&threadManagerLock) -- wake up the latest thread and schedule to run
        threadManagerLock.Unlock() -- Unlock the mutex
        endMethod
...
-----------------------------  ProcessManager  ---------------------------------
  ...
  behavior ProcessManager
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        var
          pointerToProcessControlBlock : ptr to ProcessControlBlock
        --
        -- This method returns a new ProcessControlBlock; it will wait
        -- until one is available.
        --
        processManagerLock.Lock()  -- lock the mutex

        if (freeList.IsEmpty()) -- free list is empty, you need to wait 
          -- wait on condition of process become free
          aProcessBecameFree.Wait(&processManagerLock) 
        endIf
        
        -- has some process in free list, can remove it from free list
        pointerToProcessControlBlock = freeList.Remove()
      
        -- Increment nextPid number 
        nextPid = nextPid + 1
        -- get a new process assign a unit number for id
        pointerToProcessControlBlock.pid = nextPid
        -- change status of process to active
        pointerToProcessControlBlock.status = ACTIVE
        processManagerLock.Unlock() -- unlock the mutex

        return pointerToProcessControlBlock
      endMethod
      ...
      method FreeProcess (p: ptr to ProcessControlBlock)
        ...
        processManagerLock.Lock() -- lock the mutex
        p.status = FREE           -- change status of process to free
        freeList.AddToEnd(p)      -- add that process to the free list
        aProcessBecameFree.Signal(&processManagerLock) -- do signal so another process can wake up 
        processManagerLock.Unlock() -- unlock the mutex
...
-----------------------------  InitFirstProcess  --------------------------
  ...
  function InitFirstProcess ()
    -- This routine will make a new thread, initialize it
    -- and fork to start it's running 
    var thread: ptr to Thread 

    thread = threadManager.GetANewThread() -- get a new thread
    thread.Init ("UserProgram")  -- Initialize the new thread
    thread.Fork (StartUserProcess, 0) -- Fork new thread 
...
-----------------------------  StartUserProcess  --------------------------
  ...
  var
    ...
    -- allocated new process control block with get new process 
    -- Initialize my thread in process control block by the current thread 
    -- set the process of current thread by new process control block 
    newPCB = processManager.GetANewProcess()
    newPCB.myThread = currentThread
    currentThread.myProcess = newPCB

    -- open the executable file
    open = fileManager.Open("TestProgram1")
    if open == null
      FatalError("File doesnot exist, re-check the file name")
    endIf

    -- create the logical address space 
    initPC = open.LoadExecutable(&newPCB.addrSpace)

    -- close executable file 
    fileManager.Close (open)

    -- compute the initial value for user level stack 
    -- set it to the logical address just past the end of logical address space 
    initUserStackTop = newPCB.addrSpace.numberOfPages * PAGE_SIZE

    -- reset the system stack pointer, clear the system stack
    -- jump into user level program, execution will never return 
    initSystemStackTop = &currentThread.systemStack[SYSTEM_STACK_SIZE-1]

    -- disable interrupts
    oldStatus = SetInterruptsTo (DISABLED)
    -- initialized the page table registers for this logical address
    newPCB.addrSpace.SetToThisPageTable()
    currentThread.isUserThread = true 

    -- BecomeUserThread jumps to user level main routine and never return
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
      ...
      method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
        ...
        frameManagerLock.Lock() -- Lock the mutex

        -- Wait on newFramesAvailable until there are enough 
        -- free frames to satisfy the request
        while numberFreeFrames < numFramesNeeded
          newFramesAvailable.Wait(&frameManagerLock)
        endWhile

        -- Loop through each of the frames, finding which frame is free
        -- finding the adress of that frame and storing the address of
        -- the frame
      ...
      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
        ...
        -- Get the number of frames to return
        numFramesToReturn = aPageTable.numberOfPages

        -- For each frame and frame address, calculate the bit number,
        -- clear the bit, and adjust the number of free frames
        ...
        -- Broadcast to the waiting threads so that more frames are available
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
    ...
    -- get the file name from virtual address 
    ...
    -- open the executable file
    open = fileManager.Open(&strBuffer)
    if open == null
      --FatalError("File doesnot exist, re-check the file name")
      return -1
    endIf
    ...
    -- create the logical address space 
    initPC = open.LoadExecutable(&newAddrSpace)

    -- replace old frame with new one
    frameManager.ReturnAllFrames(&currentThread.myProcess.addrSpace)

    -- be able to copy address space into ProcessControlBlock
...
-----------------------------  Handle_Sys_Create  ---------------------------------
  ...
  function Handle_Sys_Create (filename: ptr to array of char) returns int
    ...
    print ("virt addr of file name = ")
    printHex (filename asInteger)
    nl()

    print ("filename = ")
    for index = 0 to i - 1
      printChar(strBuffer[index])
    endFor
    nl()
    return 4000
  endFunction
  ...
  behavior FileManager
      ...
      method Init ()
        ...
        var i: int
          ...
          -- " Stub System " directory page.  Wed will just keep this around
